...
ResponsePacket ExampleTerminalPCSCContact::connect(const char* reader) {
	...
	if ((resp = SCardConnect(hContext_, reader, SCARD_SHARE_DIRECT, SCARD_PROTOCOL_T0 | SCARD_PROTOCOL_T1, &hCard_, &dwActiveProtocol_)) != SCARD_S_SUCCESS) {
		while (resp != SCARD_S_SUCCESS && tries < TRIES_LIMIT) {
			...
			resp = SCardConnect(hContext_, reader, SCARD_SHARE_DIRECT, SCARD_PROTOCOL_T0 | SCARD_PROTOCOL_T1, &hCard_, &dwActiveProtocol_);
			tries++;
		}
		if (resp != SCARD_S_SUCCESS) {
...
ResponsePacket ExampleTerminalPCSCContact::pollTypeAllTypes() {
	ResponsePacket response;
	response.response = "Not supported";
	return response;
}

ResponsePacket ExampleTerminalPCSCContact::getNotifications() {
	ResponsePacket response;
	response.response = "Not supported";
	return response;
}

ResponsePacket ExampleTerminalPCSCContact::clearNotifications() {
	ResponsePacket response;
	response.response = "Not supported";
	return response;
}
